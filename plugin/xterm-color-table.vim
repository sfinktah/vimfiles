...
function! <SID>ColorTable() "{{{
    ...
    for lnum in range(16, 220, 36)
        call add(rows, <SID>ColorRow(lnum, lnum + 35))
        if lnum >= 226
            call add(rows, '')
        endif
    endfor
...
function! <SID>ColorCell(n) "{{{
    ...
    execute 'syntax match bg_'.a:n.' " '.a:n.' " containedin=ALL'
    execute 'syntax match bg_'.a:n.' "'. rgb .'" containedin=ALL'

    call <SID>HighlightCell(a:n, -1)

    return printf('%4s ', a:n)
endfunction "}}}
